...
export const getWallpapers = async () => {
  try {
    ...
    const {
      ...
      axios.get(`http://www.reddit.com/r/${subreddit}/new.json?sort=new`),
      ORA_OPTIONS
    );
...
export const downloadRandomWallpaper = async (wallpapers) => {
  try {
    ...
    await imageDownloader.image({
      ...
      dest: randomWallpaper.imageDestination,
    });

    return randomWallpaper;
  } catch (error) {
    console.error('[ERROR] downloadRandomWallpaper:', error);
  }
};
...
export const switchWallpaper = async (newWallpaper) => {
  try {
    ...
    console.error('[ERROR] switchWallpaper:', error);
  }
};
